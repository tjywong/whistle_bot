...
def main():
    ...
    sub.add_parser("watch", help=f"print every message on {TOPIC}")
    args = parser.parse_args()

    link = MqttLink(args.broker, lambda p: print(f"{TOPIC}: {p}"), port=args.port)
    link.start()
    try:
        if args.cmd == "watch":
            print(f"watching {TOPIC} on {args.broker} (Ctrl+C to stop)")
            while True:
                time.sleep(1)
        payload = START if args.cmd == "start" else args.message
        link.client.publish(TOPIC, payload, qos=1).wait_for_publish(timeout=5)
        print(f"published {payload!r} to {TOPIC}")
